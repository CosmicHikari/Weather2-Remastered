...
public class EventRegisterParticleRenderer extends Event {
    ...
    public void register(ResourceLocation id, Class<?> particleRenderer) {
        if (id == null)
            ...
            Weather2.debug("Failed to register particle renderer " + id + " as the renderer was null. Skipping...");
        else if (!AbstractWeatherRenderer.class.isAssignableFrom(particleRenderer))
            Weather2.debug("Failed to register particle renderer " + id + " as the renderer does not extend from AbstractStormRenderer. Skipping...");
        else {
            registry.put(id, particleRenderer);
            Weather2.debug("Registered particle renderer " + id);
        }
    }
}