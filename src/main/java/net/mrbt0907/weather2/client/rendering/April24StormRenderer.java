...
public class April24StormRenderer extends AbstractWeatherRenderer {
    ...
    public void onTick(WeatherManagerClient manager) {
        ...
        BlockState state = ConfigCoroUtilLegacy.optimizedCloudRendering ? Blocks.AIR.defaultBlockState() : ChunkUtils.getBlockState(manager.getWorld(), (int) storm.pos_funnel_base.posX, (int) storm.pos_funnel_base.posY - 1, (int) storm.pos_funnel_base.posZ);
        ...
        if (ConfigClient.enable_tornado_block_colors) {
            if (!ConfigCoroUtilLegacy.optimizedCloudRendering && state.getBlock().equals(Blocks.AIR)) {
                state = storm.isSpout ? Blocks.WATER.defaultBlockState() : Blocks.DIRT.defaultBlockState();
                material = state.getMaterial();
            }

            if (material == Material.DIRT || material == Material.GRASS || material == Material.LEAVES || material == Material.PLANT) {
                r = 0.3F;
                g = 0.25F;
                b = 0.15F;
        ...
        if (ConfigCoroUtilLegacy.optimizedCloudRendering) {
            ...
            if (manager.getWorld().getGameTime() % (delay + ConfigClient.cloud_particle_delay) == 0) {
                for (int i = 0; i < loopSize && shouldSpawn(0); i++) {
                    if (listParticlesCloud.size() < (storm.size + extraSpawning)) {
        ...
        if (ConfigClient.enable_tornado_debris && !ConfigCoroUtilLegacy.optimizedCloudRendering && storm.stormType == StormType.LAND.ordinal() && storm.stage > Stage.SEVERE.getStage() && r >= 0.0F && !material.isLiquid()) {
        ...
        if (storm.getStage() > Stage.THUNDER.getStage() && manager.getWorld().getGameTime() % (delay + ConfigClient.cloud_particle_delay) == 0) {
            for (int i = 0; i < loopSize && shouldSpawn(4); i++) {
                if (listParticlesMeso.size() < (storm.size + extraSpawning)) {
